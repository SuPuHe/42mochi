...
async function fetchMochiStats() {
    try {
        ...
        // ❌ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Проверка на редирект аутентификации
        // Если fetch автоматически последовал за редиректом, response.url будет '/'
        if (response.url.endsWith('/') && !response.ok && response.status !== 200) {
            console.warn('Session expired or invalid. Redirecting to home page.');
            // Принудительно редиректим страницу, чтобы запустить процесс аутентификации
            window.location.href = '/';
            return;
        }

        // Если сессия недействительна и редирект не сработал (например, 401/403)
        if (!response.ok) {
            throw new Error(`Server returned status: ${response.status} on /api/getMochiStats`);
        }
